...
export default function MarketAnalysisApp() {
  ...
  useOnboardingCheck()
  const { user } = useAuth()
  const { setWorkflowData } = useWorkflow()
  ...
  const handleAnalysis = async () => {
    ...
    while (attempt < maxRetries) {
      ...
      try {
        ...
        console.log("[v0] Successfully set analysis results and saved to workflow")
      ...
      <div className="container mx-auto px-4 pb-12 max-w-6xl pt-[33px]">
        ...
        {results && (
          <div className="space-y-8 animate-in fade-in duration-700">
            ...
            {/* Workflow Navigation */}
            <WorkflowNavigation
              currentAgent={WORKFLOW_STEPS['market-analysis'].name}
              nextAgent={WORKFLOW_STEPS['market-analysis'].next}
            />
          </div>
        )}
      </div>
